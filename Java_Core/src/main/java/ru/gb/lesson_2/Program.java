...
//         if (field[0][2] == c && field[1][1] == c && field[2][0] == c) return true;
    ...
    private static boolean dig(char c) {
        for (int i = 0; i < fieldSizeX-1; i++) {
            for (int j = 0; j < fieldSizeY-1; j++) {
                if (i < fieldSizeX && j < fieldSizeY) if (field[i][j] == c && (isCellEmpty(i + 1, j + 1) || isDOT_Human(i + 1, j + 1))&&checkerOnLine()) {
                    return true;
                }
                else if (isCellEmpty(i,j)&&field[i][j] == c&&checkerOnLine()) return true;
                if (i > 1 && j  > 1) if (field[i][j] == c && (isCellEmpty(i - 1, j - 1) || isDOT_Human(i - 1, j - 1))&&checkerOnLine()) return true;
            }
        }
        return false;
    }
    private static boolean checkerOnLine(){
        int v = 0;
            for (int i = 0; i < fieldSizeX; i++) {
                for (int j = 0; j < fieldSizeY; j++) {

                    if(i==j && isDOT_Human(i,j)) {
                        if(i<fieldSizeX-1&&j<fieldSizeY-1)
                            if(i==j&&!isDOT_Human(i+1,j+1))
                                dig--;
                        dig++;
                    }if(i==j&&!isDOT_Human(i,j)){
                        dig = 0;
    ...
    private static boolean isDOT_Human(int x, int y) {
        if(field[x][y]==DOT_HUMAN) return true;
        return false;
    }
